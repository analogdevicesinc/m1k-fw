...
legend(loc='best')
figure()
semilogy(fftfreq(len(voltages_a), 2e-05), fft(voltages_a), '.', label='va')
semilogy(fftfreq(len(voltages_b), 2e-05), fft(voltages_b), '.', label='vb')
semilogy(fftfreq(len(target), 2e-05), fft(target), '.', label='dac')
legend(loc='best')
show()
